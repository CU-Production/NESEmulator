...
namespace NES
{
    public class Ppu
    {
        ...
        private bool backgroundLatchAttributeLo;
        private bool backgroundLatchAttributeHi;

        // Sprite Shift Registers
        private byte[] spriteShifterPatternLo;
        private byte[] spriteShifterPatternHi;
        private byte[] spriteXCounter;
        private OamSpriteAttributes[] spriteAttributeLatches;
        ...
        public Ppu(Emulator emulator)
        {
            ...
            spriteAttributeLatches = new OamSpriteAttributes[8];
        ...
        public void Step()
        {
            ...
            {
                // Odd Frame Idle
                if (currentCycle == 0 && currentScanline == 0 && oddFrame)
                {
                    currentCycle = 1;
                }
                ...
                // Sprite Rendering
                {
                    ...
                    else if (currentCycle == 340)
                    {
                        for (int i = 0; i < spriteCount; i++)
                        {
                            ...
                            spriteAttributeLatches[i] = sprite.attributes;
                        }
                    }
                }
            }
            ...
            if (ShouldRenderBackground())
            {
                ...
                byte pixelLSB = (byte)(((backgroundShifterPatternLo >> 8) & mux) > 0 ? 1 : 0);
                byte pixelMSB = (byte)(((backgroundShifterPatternHi >> 8) & mux) > 0 ? 1 : 0);
            ...
            if (ShouldRenderSprites())
            {
                ...
                for (int i = 0; i < spriteXCounter.Length; i++)
                {
                    if (spriteXCounter[i] == 0)
                    {
                        ...
                        spPalette = (byte)((spriteAttributeLatches[i] & OamSpriteAttributes.Palette) + 4);

                        spPriority = (spriteAttributeLatches[i] & OamSpriteAttributes.Priority) == 0;
        ...
        private void LoadBackgroundShifters()
        {
            ...
            backgroundShifterAttributeLo = (byte)(backgroundLatchAttributeLo ? 0xFF : 0x00);
            backgroundShifterAttributeHi = (byte)(backgroundLatchAttributeHi ? 0xFF : 0x00);

            backgroundLatchAttributeLo = (attributeTableByte & 0x01) > 0;
            backgroundLatchAttributeHi = (attributeTableByte & 0x02) > 0;
        }
